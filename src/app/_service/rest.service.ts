...
export class RestService {
  ...
  private getUrl(): string {
    ...
    if (null !== RestService.bankconfigurationList
      ...
      // return 'https://demobanking.synergicapi.in/utkal.synergicapi/DEMOUX/api/';
      }
    }
    return 'https://' + 'demobanking.synergicapi.in'+ '/' + __bName + '/api/'
  ...
  // public addUpdDel<T>(ofwhat: string, data: T): Observable<T> {
  //   const encryptedPayload = { data: this.encryptionService.encryptData(data) };

  //   return this.http.post<{ data: string }>(this.getUrl() + ofwhat, encryptedPayload).pipe(
  //     map(response => {
  //       if (response && response.data) {
  //         return this.encryptionService.decryptData(response.data) as T;
  //       }
  //       return null;
  //     })
  //   );
  // }
  public getURL<T>(ofwhat: string): Observable<T> {
    // console.log(data)
    // console.log(ofwhat)
