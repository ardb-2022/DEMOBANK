...
getImage(_custCD: number) {
  ...
  CheckExistance(value,_flag){
    console.log(value,_flag);
    
    var dt = {
         'kyc':value,
         'flag': _flag
    }
    this.svc.addUpdDel<any>('UCIC/DuplicateKYC', dt).subscribe(
      res => {
                console.log(res);
                this.isLoading = false;
                if(res > 0){
                    ...
                    //  _mode == 'PIN' ? this.kycPhotoNo.nativeElement.focus() : this.kycAddressNo.nativeElement.focus()
                     this.HandleMessage(true, MessageType.Error, _flag == 'K' ? `This KYC already exists for UCIC- ${res}` 
                     :`This Mobile number already exists for UCIC- ${res}`);  
