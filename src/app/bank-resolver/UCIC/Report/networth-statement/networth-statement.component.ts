...
export class NetworthStatementComponent implements OnInit {
  ...
  config = {
   ...
   totalDepositOpenBal:number=0
   totalDepositCloseBal:number=0
   totalLoanOpenBal:number=0;
   totalLoanCloseBal:number=0;
  showReport = false;
  showAlert = false;
  isLoading = false;
  ...
  public SubmitReport() {
    if (this.reportcriteria.invalid) {
      ...
      this.modalRef.hide();
      this.totalDepositOpenBal=0
      this.totalDepositCloseBal=0
      this.totalLoanOpenBal=0;
      this.totalLoanCloseBal=0;
      ...
      this.svc.addUpdDel('UCIC/GetLoanDtls',dt).subscribe(data=>{console.log(data)
        ...
        for(let i=0;i<this.reportData.length;i++){
          ...
          if(this.reportData[i]?.loan_status=='O'){
              this.totalLoanOpenBal+=(+this.reportData[i]?.penal_intt)+(+this.reportData[i]?.ovd_intt)+(+this.reportData[i]?.curr_intt)+(+this.reportData[i]?.ovd_prn)+(+this.reportData[i]?.curr_prn)
            }
            else{
              this.totalLoanCloseBal+=(+this.reportData[i]?.penal_intt)+(+this.reportData[i]?.ovd_intt)+(+this.reportData[i]?.curr_intt)+(+this.reportData[i]?.ovd_prn)+(+this.reportData[i]?.curr_prn)
            }  
        ...
        this.svc.addUpdDel('UCIC/GetDepositDtls',dt).subscribe(data=>{console.log(data)
          ...
          for(let i=0;i<this.reportData1.length;i++){
            debugger
            if(this.reportData1[i].acc_status=='O'){
              this.totalDepositOpenBal+=this.reportData1[i]?.clr_bal;
              debugger
            }
            else{
              this.totalDepositCloseBal+=this.reportData1[i]?.clr_bal;
              debugger
            }
          ...
          if(!this.reportData && this.reportData1.length==0){
