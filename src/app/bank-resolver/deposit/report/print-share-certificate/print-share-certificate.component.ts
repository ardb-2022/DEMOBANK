...
import { SystemValues, p_report_param, mm_customer, tm_deposit, mm_operation, mm_acc_type } from 'src/app/bank-resolver/Models';
...
@Component({
  selector: 'app-print-share-certificate',
  templateUrl: './print-share-certificate.component.html',
  styleUrls: ['./print-share-certificate.component.css'],
  providers:[ExportAsService]
})
export class PrintShareCertificateComponent implements OnInit {
  ...
  extractDatePart(dateStr: any) {
    console.log(dateStr);
    debugger
  return dateStr.split(' ')[0]; // Splits "19/04/2018 00:00" → ["19/04/2018", "00:00"]
}
   ...
   getAccountTypeList() {
    ...
    this.svc.addUpdDel<any>('Mst/GetAccountTypeMaster', dt).subscribe(
      res => {
        ...
        this.AcctTypes = res;
        this.AcctTypes2 =this.AcctTypes.filter(e=>e.acc_type_cd==8 )
  ...
  public suggestCustomer(): void {
    ...
    if (this.reportcriteria.controls.acct_num.value.length > 0) {
      ...
      this.svc.addUpdDel<any>('Deposit/GetAccDtls', prm).subscribe(
        ...
        err => { this.isLoading = false;  this.showWait=false}
      );
    } else {
       this.showWait=false
      this.suggestedCustomer = null;
      ...
      this.svc.addUpdDel<any>('Deposit/GetAccountOpeningData', this.tm_deposit).subscribe(
        res => {
          ...
          if(this.masterModel?.tmdeposit?.acc_num!=null){
            ...
            this.openingDT=this.extractDatePart(this.masterModel.tmdeposit.opening_dt);
            this.shareAmt=this.masterModel.tmdeposit.clr_bal;
            this.issuDate=this.formatDateToDDMMYYYY(this.today);
          debugger
          }
