...
@Component({
  selector: 'app-icic-vender-list',
  templateUrl: './icic-vender-list.component.html',
  styleUrls: ['./icic-vender-list.component.css'],
  providers:[ExportAsService]
})
export class ICICVenderListComponent implements OnInit ,AfterViewInit{ 
  @ViewChild('content', { static: true }) content: TemplateRef<any>;
@ViewChild(MatPaginator) paginator: MatPaginator;
@ViewChild(MatSort) sort: MatSort;
dataSource = new MatTableDataSource()
displayedColumns: string[] = ['record_Identifier','customer_Code','customer_Name'];
...
public SubmitReport() {
  ...
  if (this.reportcriteria.invalid) {
    ...
    this.svc.addUpdDel('Deposit/GetICICIVendorList',dt).subscribe(data=>{console.log(data)
...
downloadexcel(){
  ...
  this.exportAsService.save(this.exportAsConfig, 'VendorList').subscribe(() => {
    // save started
    console.log("hello")
  });
}
...
applyBranchFilter(event: Event){
const filterValue = (event.target as HTMLInputElement).value;
  this.dataSource.data=this.reportData2.filter(e=>e.brn_cd==filterValue.trim().toLowerCase())
