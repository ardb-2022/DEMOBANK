...
export class PrintCertificateComponent implements OnInit {
  ...
  //         this.selectedAccNature = this.AcctTypes.filter(x => x.acc_type_cd?.toString() == accType?.toString())[0]?.dw_obj;
  ...
  public SubmitReport() {
    this.allNominee='';
   if(this.reportcriteria.controls.acc_type_cd.value=='5'||this.reportcriteria.controls.acc_type_cd.value=='16'){
      this.ShowMIS=true;
      this.ShowCC=false;
