...
export class DdsAccStmtComponent implements OnInit ,AfterViewInit{
  ...
  // displayedColumns: string[] = ['paid_dt','deposit_amt','withdrawal_amt','closing_bal'];
  allAgent:any[]=[];
  modalRef: BsModalRef;
  isOpenFromDp = false;
  isOpenToDp = false;
  ...
  public SubmitReport() {
    if (this.reportcriteria.invalid) {
      ...
      console.log(this.reportData[0]?.closing_bal);
      if(!this.reportData){
      ...
      this.total_bal+=this.reportData[i].deposit_amt
        this.opdrSum+=this.reportData[i].withdrawal_amt
      // console.log(this.reportData[i].dr_amt)
    
      // this.crSum+=this.reportData[i].cr_amount
    }
 ...
 closeScreen() {
  ...
  getTotal(){
    ...
    for(let i=0;i<this.filteredArray.length;i++){
      ...
      this.total_bal+=this.filteredArray[i].deposit_amt
        this.opdrSum+=this.filteredArray[i].withdrawal_amt
