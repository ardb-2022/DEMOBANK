...
export class PassBookFastPageComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.getsystemParam();
    if(this.accType=="6"  ){
      this.printID="fastPageRDPuri"
    }else{
      this.printID="fastPagePuri"
      // Ghatal
    }
    debugger
  ...
  loadFastPageData(){
    ...
    this.svc.addUpdDel('Deposit/getAccountOpeningData',dt).subscribe(data=>{
     ...
     if(data){
      ...
      this.getCustomer()
       this.selectedConst=""
        this.selectedConst =this.constitutionList.filter(e => e.constitution_cd
                    === this.masterModel?.tmdeposit?.constitution_cd)[0]?.constitution_desc;
        ...
        for (let i = 0; i < this.masterModel.tdaccholder.length; i++) {
          const holder = this.masterModel.tdaccholder[i];

          if (this.masterModel.tdaccholder.length === 0) {
            continue;
          } else if (this.masterModel.tdaccholder.length === 1) {
            if (holder?.cust_cd > 0) {
              this.joinHold += holder.acc_holder;
            }
          } else {
            this.joinHold += holder.acc_holder;
            if (i !== this.masterModel.tdaccholder.length - 1) {
              this.joinHold += ',';
            }
          }
        }
        if (this.masterModel.tdnominee.length > 0) {
          this.AllNominee = this.masterModel.tdnominee
            .map(n => n?.nom_name || '')
