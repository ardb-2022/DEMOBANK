    ...
    if (this.reportcriteria.invalid) {
      ...
      this.svc.addUpdDel('Deposit/PopulateDLDds',dt).subscribe(data=>{
        ...
        this.reportData.forEach(e=>{
          for (let i = 0; i < e.ttsbcadtllist.length; i++) {
            e.ttsbcadtllist[i].opening_dt=this.comSer.getFormatedDate(e.ttsbcadtllist[i].opening_dt);
            e.ttsbcadtllist[i].mat_dt=this.comSer.getFormatedDate(e.ttsbcadtllist[i].mat_dt);
          }
