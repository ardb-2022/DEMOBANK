 ...
 deleteuser() {
  ...
  retrieveData(){
    ...
    //     this.isApprove=true;
        ...
        // }
        this.svc.addUpdDel<any>('Deposit/GetUnapprovedDdsTransAndroid',dt).subscribe(dat=>{
          console.log(dat);
          this.isLoading=false;
          this.isApprove=true;
            this.transData=dat;
  ...
  addNewRow() {
    if(this.transData.length>0){
      this.newRow = {
          ...
          cust_name: null,
          paid_amt: 0,
          paid_dt:this.sys.CurrentDate,
          trans_cd: this.transData[0]?.trans_cd,
          trans_type: "D"
        };
    }
    else{
  ...
  this.transData.push(this.newRow);
