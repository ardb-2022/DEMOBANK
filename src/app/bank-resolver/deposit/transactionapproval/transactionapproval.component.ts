...
export class TransactionapprovalComponent implements OnInit {
  ...
  public selectTransaction(vm: TranApprovalVM): void {
    ...
    this.getAgentName(vm.td_def_trans_trf.acc_num)
    debugger
  ...
  getAgentName(i:any){
     this.svc.addUpdDel<any>(`Deposit/GetAgentDetails?accNum=${+i}`, null).subscribe(
        res => {
          if(res!='000000'){
            this.accDtlsFrm.controls.agnt_name.setValue(res)
              this.agentNameForAppACC=true

          }else{
            this.accDtlsFrm.controls.agnt_name.setValue(null)
            this.agentNameForAppACC=false
          }
        },
        err=>{
           if(err.error.text!='000000'){
            this.accDtlsFrm.controls.agnt_name.setValue(err.error.text)
              this.agentNameForAppACC=true
  ...
  private GetUnapprovedDepTrans(): void {
    ...
    this.svc.addUpdDel<any>('Common/GetUnapprovedDepTrans', this.tdDepTrans).subscribe(
      res => {
        ...
        this.vm.forEach(e=>{
          if(e.td_def_trans_trf.created_by){
