...
export class AccOpeningComponent implements OnInit {
  ...
  disabledTransaction:boolean=false;
...
reportData1:any[]=[];
  ...
  saveData() {
    ...
    if(this.tm_deposit.acc_type_cd){
  ...
  setAccountType(accType: number) {
    ...
    if((accType)!=1){
      this.svc.addUpdDel<any>('UCIC/GetDepositDtls',dt).subscribe(data=>{console.log(data)
        ...
        if(this.reportData1){
          debugger
          const hasAccType1 = this.reportData1.some(item => item.acc_type_cd != 1);
          debugger
          // if(hasAccType1){
          //   this.tm_deposit.acc_type_cd='';
          //   this.HandleMessage(true, MessageType.Warning, 'Please create at least one savings account....');
          // return 
          // }
 ...
 SetInttTfrType(_opern_Type:any,_acc_type_cd:any){
  ...
  public setCustDtls(cust_cd: number): void {
    this.disabledTransaction=false;
  ...
  populateCustDtls(temp_mm_cust: mm_customer) {
    if(temp_mm_cust.dt_of_birth?.toString() =='01/01/0001 00:00' || (!temp_mm_cust.occupation || !temp_mm_cust.sex)){
      this.HandleMessage(true, MessageType.Error, `For that customer account can't open beacause, customer information like (DOB/SEX/OCCUPATION) not updated`);
      this.disabledTransaction=true;
    }
  ...
  processInstallmentAmount() {
    const dt={
                "AccTypeCd": this.tm_deposit.acc_type_cd,
                "CatgCd": this.tm_deposit.category_cd,
                "NoOfDays": Math.floor((Date.UTC(this.tm_deposit.mat_dt.getFullYear(),
                            this.tm_deposit.mat_dt.getMonth(), this.tm_deposit.mat_dt.getDate()) -
                            (Date.UTC(this.tm_deposit.opening_dt.getFullYear(), this.tm_deposit.opening_dt.getMonth(),
                            this.tm_deposit.opening_dt.getDate()))) / (1000 * 60 * 60 * 24)),
                "Amount": this.tm_deposit.acc_type_cd==6?this.tm_deposit.instl_amt:this.tm_deposit.prn_amt,
                "EffDt": this.sys.CurrentDate,
                "IsLady": this.tm_deposit.cust_type=='B'?'N':this.tm_deposit.sex=='M'?'N':'Y',
                "IsRetired": this.tm_deposit.cust_type=='B'?'N':this.tm_deposit.occupation=='RET_GOV_EMPLOYEE'?'Y':'N',
                "IsChild": this.tm_deposit.cust_type=='B'?'N':this.calculateAge(this.tm_deposit.date_of_birth) >=12?'N':'Y',
                "IsFarmer": this.tm_deposit.cust_type=='B'?'N':this.tm_deposit.occupation=='FARMER'?'Y':'N',
              }
 this.svc.addUpdDel<any>('Deposit/GetInterestRate', dt).subscribe(
        res => {
          //  if(res){
          if(res.status=='Success'){
            const rt=res.data[0]?.interestRate;
            this.tm_deposit.intt_rt = Number(rt);
            ...
            temp_gen_param.an_intt_rate = Number(rt);
            ...
            this.calCrdIntReg(temp_gen_param);
             ...
             this.HandleMessage(true, MessageType.Error, 'Please Update the Required Customer Information');
               return;
          }
  ...
  processYearMonthDay() {
    ...
    if (this.operationType === 'I' && this.selectedAccNature!='SB' && this.selectedAccNature!='DD'){
  ...
  calculateInterestRate() {
   const dt={
                ...
                "EffDt": this.sys.CurrentDate,
                "IsLady": this.tm_deposit.cust_type=='B'?'N':this.tm_deposit.sex=='M'?'N':'Y',
                "IsRetired": this.tm_deposit.cust_type=='B'?'N':this.tm_deposit.occupation?.toUpperCase()=='RET_GOV_EMPLOYEE'?'Y':'N',
                "IsChild": this.tm_deposit.cust_type=='B'?'N':this.calculateAge(this.tm_deposit.date_of_birth) >=12?'N':'Y',
                "IsFarmer": this.tm_deposit.cust_type=='B'?'N':this.tm_deposit.occupation?.toUpperCase()=='FARMER'?'Y':'N',
              }
 this.calculateAge(this.tm_deposit.date_of_birth)
