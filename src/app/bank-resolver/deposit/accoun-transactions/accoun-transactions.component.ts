...
getRDActiveList(){
  ...
  calculateInterestRate() {
    ...
    const o = Utils.convertStringToDt(this.td.opening_dt.value);
      ...
      // this.calculateInttRT = this.getInterestRate(this.td.opening_dt.value,this.td.mat_dt.value)
      this.getInterestRate(this.td.opening_dt.value, this.td.mat_dt.value)
        .then(rate => {
          this.calculateInttRT = rate;
          this.td.intt_rate.setValue(rate);
        });
      debugger
      if(this.calculateInttRT>0){
        const temp_gen_param2 = new p_gen_param();
          temp_gen_param2.ardb_cd=this.sys.ardbCD;
          ...
          temp_gen_param2.ad_intt_rt = this.calculateInttRT;
        ...
        this.HandleMessage(true, MessageType.Error, 'Can not get Interest Rate, Please Update Master Chart');
      }
...
debugger
      ...
      temp_gen_param.ad_prn_amt = this.td.acc_type_cd.value === 5?this.td.td_def_mat_amt.value:this.td.amount.value;
...
debugger
  ...
  private async setAccDtlsFrmForm(): Promise<void> {
    // //////////////debugger;

    if (undefined !== this.accNoEnteredForTransaction && Object.keys(this.accNoEnteredForTransaction).length !== 0) {
      this.resetAccDtlsFrmFormData();
      this.getShadowBalance();
      this.allChequeData=await this.getChequeIssueData(this.accNoEnteredForTransaction.acc_type_cd,this.accNoEnteredForTransaction.acc_num);
      console.log(
      await this.getChequeIssueData(this.accNoEnteredForTransaction.acc_type_cd,this.accNoEnteredForTransaction.acc_num)
      );
...
// tdaccholder[0]?.acc_holder
  ...
  private validationOnAcctTabOff(acc: tm_depositall): boolean {
    ...
    // }
    // acc.acc_type_cd === 5 ||
    if((acc.acc_type_cd === 2||acc.acc_type_cd === 3||acc.acc_type_cd === 4|| acc.acc_type_cd === 5 ||acc.acc_type_cd === 6 ||acc.acc_type_cd === 10||acc.acc_type_cd === 14) && acc.lock_mode){
    ...
    if (acc.acc_type_cd === 1 || acc.acc_type_cd === 7 || acc.acc_type_cd === 8 || acc.acc_type_cd === 11 || acc.acc_type_cd === 12 || acc.acc_type_cd === 13 ) {
    ...
    switch (acc.acc_type_cd) {
      ...
      this.misSum=0
        this.showMisInstalment = false;
        const miisInstalments = new td_intt_dtls();
        miisInstalments.ardb_cd = this.sys.ardbCD
        miisInstalments.brn_cd = this.sys.BranchCode;
        miisInstalments.acc_num = acc.acc_num;
        miisInstalments.acc_type_cd = acc.acc_type_cd;
        this.svc.addUpdDel<any>('Deposit/GetInttDetails', miisInstalments).subscribe(
          misInstalmentsRes => {
            console.log(misInstalmentsRes)
            this.misInstallemntsForSelectedAcc = [];
            this.misInstallemntsForSelectedAcc = Utils.ChkArrNotEmptyRetrnEmptyArr(misInstalmentsRes);
  ...
  hideModalForClose() {
    ...
    if(+this.f.acc_type_cd.value == 2){
      ...
      debugger
      if( Number(this.f.acc_type_cd.value) == 4  )
        {
                              debugger
                              this.tdDefTransFrm.patchValue({
                                ovd_intt_recov: 0,
       ...
       if(  Number(this.f.acc_type_cd.value) == 10  )
        {
                              debugger
                              this.tdDefTransFrm.patchValue({
                                ovd_intt_recov: 0,
                                ...
                                bonus_amt: 0,
                                amount: (+this.accNoEnteredForTransaction.prn_amt),
                                curr_intt_recov:(this.afMat1?(+this.interestAmount)+(+this.aftmatInt):(+this.closeInt))+this.accNoEnteredForTransaction.intt_amt ,
                                td_def_mat_amt:(this.afMat1?this.accNoEnteredForTransaction.prn_amt + this.interestAmount+(+this.aftmatInt):this.accNoEnteredForTransaction.prn_amt +(+this.closeInt))+this.accNoEnteredForTransaction.intt_amt,
                                });
                            }          
      if((Number(this.f.acc_type_cd.value) == 5 ||Number(this.f.acc_type_cd.value) == 16) && this.aftmatInt)
      {
        debugger
        this.tdDefTransFrm.patchValue({
          ovd_intt_recov: 0,
  ...
  hideModalForRenewal(intt){
    this.modalRefClose.hide();
     if(Number(this.f.acc_type_cd.value) == 5||Number(this.f.acc_type_cd.value) == 16 )
      {
          this.tdDefTransFrm.patchValue({
          ovd_intt_recov: 0,
          curr_prn_recov: 0,
          bonus_amt: 0,
          amount: (+this.accNoEnteredForTransaction.prn_amt),
          curr_intt_recov:(+intt),
          td_def_mat_amt:this.accNoEnteredForTransaction.prn_amt + (+intt)
          });
        }
  ...
  public async onOperationTypeChange(): Promise<void> {
    ...
    if (selectedOperation.oprn_desc.toLocaleLowerCase() === 'withdraw') {
        ...
        if(this.RDAllInstlPaid){
          ...
          if(this.StandingRD.length>0){
            ...
            if(this.StandingArr2){
              this.HandleMessage(true, MessageType.Warning, `A/C ${this.f.acct_num.value} have already Standing Instraction, be cautious`);
              // this.showTransactionDtl=false;
              // return
            }
          }
      ...
      debugger
       this.aftmatInt=0;
       if((this.sys.ardbCD!='20'&& this.sys.ardbCD!='26') && ( this.afMat1 == true && (accTypCode === 2 ||accTypCode === 3 || accTypCode === 4|| accTypCode === 5||accTypCode === 14 ||accTypCode === 16||accTypCode === 10  ) )) 
        {
        ...
        const temp_gen_param = new p_gen_param();
            ...
            // const data = await this.svc.addUpdDel<any>('Deposit/F_CALCTDINTT_REG', temp_gen_param).subscribe(
              ...
              if(data) {
                ...
                if(data){
                  debugger
                  if((Number(this.f.acc_type_cd.value) == 4) && this.sys.ardbCD=='1' ){
                              debugger
                              this.showOnClose = true;
                              this.tdDefTransFrm.patchValue({
                                ovd_intt_recov: 0,
 ...
 const dt={
                "AccTypeCd": this.accNoEnteredForTransaction.acc_type_cd,
                "CatgCd": this.accNoEnteredForTransaction.catg_cd,
                "NoOfDays": this.diff,
                "Amount": this.accNoEnteredForTransaction.acc_type_cd==6?this.accNoEnteredForTransaction.instl_amt:this.accNoEnteredForTransaction.prn_amt,
                "EffDt": this.accNoEnteredForTransaction.opening_dt,
                "IsLady": this.accNoEnteredForTransaction.cust_type=='B'?'N':this.accNoEnteredForTransaction.sex=='M'?'N':'Y',
                "IsRetired": this.accNoEnteredForTransaction.cust_type=='B'?'N':this.accNoEnteredForTransaction.occupation?.toUpperCase()=='RET_GOV_EMPLOYEE'?'Y':'N',
                "IsChild": this.accNoEnteredForTransaction.cust_type=='B'?'N':this.calculateAge(this.accNoEnteredForTransaction.dt_of_birth) >=12?'N':'Y',
                "IsFarmer": this.accNoEnteredForTransaction.cust_type=='B'?'N':this.accNoEnteredForTransaction.occupation?.toUpperCase()=='FARMER'?'Y':'N',
              }
     
            debugger
            this.isLoading=true;
            this.svc.addUpdDel<any>('Deposit/GetInterestRate', dt).subscribe(
            resrt => {
              console.log(resrt)
              this.isLoading=false;
              if(resrt.status=='Success'){
            const rt=resrt.data[0]?.interestRate;
            if(this.isMat==false && this.accNoEnteredForTransaction.acc_type_cd!==6 ){
                this.effInt = Number(rt) -1;
            }else{
               this.effInt = Number(rt) 
            }
...
// // this.isLoading = true;
      if (isMatured == false && (accTypCode === 2 || accTypCode == 4 ||accTypCode == 3||accTypCode == 10)) {
        this.modalRefClose = this.modalService.show(this.preClose,
          { class: 'modal-lg', keyboard: false, backdrop: true, ignoreBackdropClick: true })
      }
      ...
      if (accTypCode === 5 ||accTypCode === 16 || (accTypCode==2 && this.resbrnCD1[0]?.intt_trf_type=='H' || accTypCode==2 && this.resbrnCD1[0]?.intt_trf_type=='Q' || accTypCode==2 && this.resbrnCD1[0]?.intt_trf_type=='Y')) { // Special logic for MIS on close
              ...
              res_rt => {
                ...
                this.svc.addUpdDel<any>('Deposit/F_CALCTDINTT_REG', temp_gen_param).subscribe(
                  result => {
                    ...
                    //   this.td.amount.disable();
                    // }
                    if (isMatured == false && ((accTypCode == 5)||accTypCode === 16)) {
                      isMatured == false && (accTypCode == 5||accTypCode === 16)?this.misPreMatClose=true:this.misPreMatClose=false;
                      this.modalRefClose = this.modalService.show(this.preClose,
                        { class: 'modal-lg', keyboard: false, backdrop: true, ignoreBackdropClick: true })
                    }
                    if(accTypCode==5 ||accTypCode === 16){//MISCLOSE
                      this.tdDefTransFrm.patchValue({
                        interest: 0,
                        // amount: this.accNoEnteredForTransaction.prn_amt
                        // + this.accNoEnteredForTransaction.intt_amt,
      ...
      this.tdDefTransFrm.controls.amount.disable()
        ...
        if (this.custBrnCd != this.sys.BranchCode) {
          ...
          console.log(this.td.intt_rate.value);
...
debugger
          ...
          if ((accTypCode === 5||accTypCode === 16) || (accTypCode === 2 && this.resbrnCD1[0]?.intt_trf_type=='O')){ // Special logic for MIS on renewal
            console.log(this.diff)
            debugger
            this.tdDefTransFrm.patchValue({
              intt_trf_type: (accTypCode==5||accTypCode == 16)? 'M':'O'
            })
            ...
            this.svc.addUpdDel<any>('Deposit/GetInttDetails', misInstalments).subscribe(
              misInstalmentsRes => {
                ...
                if((accTypCode==5||accTypCode==16) && this.tdDefTransFrm.controls.trans_mode.value=='R' && !this.editDeleteMode){
                  this.showOnClose = false; 
                  debugger
                  this.td.amount.enable();
                  this.td.amount.setValue(mat_amt_mis);
      ...
      if (this.diff1>15) {
        ...
        if( (afterMatured == true && ((accTypCode === 2 && this.resbrnCD1[0]?.intt_trf_type=='O') || accTypCode == 4||accTypCode == 3||accTypCode == 5))) {

        //  if(afterMatured == true && (accTypCode === 2 || accTypCode == 4)) {
          this.modalRefClose = this.modalService.show(this.afterMatRenewal,
            { class: 'modal-lg', keyboard: false, backdrop: true, ignoreBackdropClick: true })
        
            const prn_amt_for_renew=(accTypCode === 5 )?
...
debugger
    ...
    CloseDPenalty(){
      ...
      if(this.f.acc_type_cd.value==5 ||this.f.acc_type_cd.value==16 ){
        this.tdDefTransFrm.patchValue({
          ...
          "from_dt":this.sys.CurrentDate
                ...
                this.svc.addUpdDel<any>('Deposit/GetDdsInterest', dt).subscribe(
                  (res:number) => {
                    console.log(res)
                    if (res>0) {
                        alert(' Mature Closing Interest Received RS:'+(res))
                        this.isLoading=false
                        this.rdInClose=res;
                        debugger
                      this.tdDefTransFrm.patchValue({
                      ...
                      alert('It will be a premature Closing RS:'+(res)+' Panalty received')
                      ...
                      this.tdDefTransFrm.patchValue({
                        ...
                        amount:+this.accNoEnteredForTransaction.prn_amt,
                        curr_intt_recov: 0,
                        td_def_mat_amt: ((+(+this.accNoEnteredForTransaction.prn_amt)-(+positiveValue.toFixed(2)))),
                      });
  ...
  public async getInterestRate(openDT: any, matDT: any): Promise<number> {
  const o = Utils.convertStringToDt(openDT);
  const m = Utils.convertStringToDt(matDT);
  const diffDays = Math.ceil((Math.abs(m.getTime() - o.getTime())) / (1000 * 3600 * 24));
  ...
  const dt={
                ...
                "CatgCd": this.accNoEnteredForTransaction.catg_cd,
                "NoOfDays": diffDays,
                "Amount": this.accNoEnteredForTransaction.acc_type_cd==6?this.accNoEnteredForTransaction.instl_amt:this.td.amount.value,
                "EffDt": this.accNoEnteredForTransaction.mat_dt,
                "IsLady": this.accNoEnteredForTransaction.cust_type=='B'?'N':this.accNoEnteredForTransaction.sex=='M'?'N':'Y',
                "IsRetired": this.accNoEnteredForTransaction.cust_type=='B'?'N':this.accNoEnteredForTransaction.occupation?.toUpperCase()=='RET_GOV_EMPLOYEE'?'Y':'N',
                "IsChild": this.accNoEnteredForTransaction.cust_type=='B'?'N':this.calculateAge(this.accNoEnteredForTransaction.dt_of_birth) >=12?'N':'Y',
                "IsFarmer": this.accNoEnteredForTransaction.cust_type=='B'?'N':this.accNoEnteredForTransaction.occupation?.toUpperCase()=='FARMER'?'Y':'N',
  ...
  try {
    const resrt = await this.svc.addUpdDel<any>('Deposit/GetInterestRate', dt).toPromise();
    if (resrt.status === 'Success') {
      return Number(resrt.data[0]?.interestRate) || 0;
    }
    return 0;
  } catch (err) {
    console.log(err);
...
//   public getInterestRate(openDT,matDT){
//       const o = Utils.convertStringToDt(openDT);
//       const m = Utils.convertStringToDt(matDT);
//       const diffDays = Math.ceil((Math.abs(m.getTime() - o.getTime())) / (1000 * 3600 * 24));
//       // temp_gen_param.ai_period = diffDays;
...
//  }
  ...
  calculateAge(input: string | Date): number | null {
  if (!input) return null;
  // if(input=="01/01/0001 00:00") return null;
  let birthDate: Date;

  if (input instanceof Date) {
    // Directly use if it's already a Date object
    birthDate = input;
  } else {
...
debugger
          ...
          res => {
            if (undefined !== res && null !== res && !isNaN(+res)) {
              ...
              if (shadowBalance - (+this.td.amount.value) < 0) {
               ...
               else if (shadowBalance - (+this.td.amount.value) < minBal && (accTypeCd === 1 )&& this.sys.ardbCD=='1') {
                  this.HandleMessage(true, MessageType.Error, 'Withdrawal Amount is executed your A/C minimum balance ' + minBal + '.');
                  this.td.amount.setValue('');
                  return;
                  // const cnfrm = confirm('Amount is less than minimum balance ' + minBal + '. Press Ok to continue, else Cancel');
                ...
                else if(this.sys.ardbCD=='1' && (accTypeCd==1) && this.td.trans_type_key.value=='W'){
                  let minBal = 0;
                  minBal=this.accNoEnteredForTransaction.ext_instl_tot
                  debugger
                  if (shadowBalance - (+this.td.amount.value) < minBal) {
  ...
  onSaveClick(): void {
    ...
    debugger;
      if(this.f.acc_type_cd.value == 11 && this.td.trf_type.value === 'T' && this.showtransdetails == true && this.TrfTotAmt !== (+this.tdDefTransFrm.controls.amount.value+(+this.tdDefTransFrm.controls.curr_intt_recov.value))-(+this.tdDefTransFrm.controls.ovd_intt_recov.value)){
    ...
    if (!this.editDeleteMode) {
      ...
      if (this.td.trf_type.value === 'C') {
        ...
        this.transactionAmount=(this.td.acc_type_cd.value== 1
          ||this.td.acc_type_cd.value == 7
          ||this.td.acc_type_cd.value == 13
          ||this.td.acc_type_cd.value == 8
          ||this.td.acc_type_cd.value == 9
          ||this.td.acc_type_cd.value == 12)?
          this.td.amount.value:this.td.td_def_mat_amt.value;
        this.cashMode=true;
      } else if (this.td.trf_type.value === 'T') {
        let i = 0;
      ...
      if (this.td.trf_type.value === 'C') {
        ...
        this.td_deftranstrfList.forEach(e => {
          if (e.amount != null) {
            ...
            tdDefTransAndTranfer.amount = e.amount;
            tdDefTransAndTranfer.home_brn_cd = this.sys.ardbCD=='1'? tdDefTransAndTranfer.acc_num.substring(2, 3):tdDefTransAndTranfer.acc_num.substring(0, 3);
            tdDefTransAndTranfer.intra_branch_trn = this.sys.ardbCD=='1'? tdDefTransAndTranfer.acc_num.substring(2, 3):tdDefTransAndTranfer.acc_num.substring(0, 3) != this.sys.BranchCode ? 'Y' : 'N'
            console.log(tdDefTransAndTranfer.particulars)
            //  debugger;
  ...
  mappTddefTransFromFrm(): td_def_trans_trf {
    ...
    if (!this.editDeleteMode) {
      ...
      if (selectedOperation.oprn_desc.toLocaleLowerCase() === 'close' && (accTypeCd === 2 || accTypeCd === 3 || accTypeCd === 4 || accTypeCd === 5 || accTypeCd === 6 || accTypeCd === 10|| accTypeCd === 11 || accTypeCd === 16)) {
      debugger
       ...
       if(accTypeCd==10){
        ...
        toReturn.amount = ((+this.tdDefTransFrm.controls.amount.value) + (+this.tdDefTransFrm.controls.curr_prn_recov.value))-(+this.tdDefTransFrm.controls.ovd_intt_recov.value);
        ...
        toReturn.ovd_intt_recov = (accTypeCd === 5 || accTypeCd === 16) ? 0 : +this.td.ovd_intt_recov.value;
        ...
        if (selectedOperation.oprn_desc.toLocaleLowerCase() === 'renewal') {
          ...
          // debugger;
          for (let i = 0; i < AccounTransactionsComponent.constitutionList.length; i++) {
            if (AccounTransactionsComponent.constitutionList[i].constitution_cd == this.constCd && AccounTransactionsComponent.constitutionList[i].acc_type_cd == accTypeCd && (this.td.trans_type.value.toLocaleLowerCase() == 'withdraw')) {
              toReturn.acc_cd = AccounTransactionsComponent.constitutionList[i].acc_cd
              console.log(toReturn.acc_cd)
              // debugger;
            }
            if (AccounTransactionsComponent.constitutionList[i].constitution_cd == this.constCd && AccounTransactionsComponent.constitutionList[i].acc_type_cd == accTypeCd && (this.td.trans_type.value.toLocaleLowerCase() == 'deposit')) {
              toReturn.acc_cd = AccounTransactionsComponent.constitutionList[i].acc_cd
              console.log(toReturn.acc_cd)
              //  debugger;
            }
          } //marker
          const desc = accTypeCd == 2 ? 'FD ' : (accTypeCd === 3 ? 'DBS' : (accTypeCd == 4 ? 'TD' : (accTypeCd == 5 ? 'MIS ' : (accTypeCd == 6 ? 'RD ' : (accTypeCd == 7 ? 'Current ' : (accTypeCd == 8 ? 'Share ': (accTypeCd == 10 ? 'JBB ':(accTypeCd == 11 ? 'DDS ':(accTypeCd == 16 ? 'MIIS ' : 'DSP')))))))))
      ...
      ///here else if is made a marker for making intt cd 
        ...
        toReturn.curr_prn_recov = (accTypeCd === 5||accTypeCd===16) ? this.td.td_def_mat_amt.value: ((+this.td.amount.value) + (+this.td.interest.value));
        toReturn.ovd_prn_recov = (accTypeCd === 5 ||accTypeCd===16)? this.accNoEnteredForTransaction.prn_amt : this.accNoEnteredForTransaction.prn_amt;
        toReturn.curr_intt_recov = (accTypeCd === 5 ||accTypeCd===16)? (this.matInt>0? +this.misSum+this.matInt:this.misSum): this.accNoEnteredForTransaction.intt_amt;
        ...
        if(accTypeCd==2 && this.accDtlsFrm.controls.intt_trf_type.value.toLowerCase()=='on maturity'){
          ...
          toReturn.acc_cd = this.resbrnCD1[0]?.acc_cd; //partha
          debugger
        }
...
debugger
      ...
      if (this.td.trans_mode.value && this.td.trans_mode.value.toLocaleLowerCase() == 'r' && (!this.afMat)){
        if (this.td.acc_type_cd.value != 5 && this.td.acc_type_cd.value != 16)
          toReturn.amount = this.tdDefTransFrm.controls.amount.value == this.accDtlsFrm.controls.mat_amt.value ? this.accDtlsFrm.controls.intt_amt.value : 0
        if (this.td.acc_type_cd.value == 5 ||this.td.acc_type_cd.value == 16)
          toReturn.amount = 0
      }
  ...
  mappTddefTransAndTransFrFromFrm(): td_def_trans_trf {
    ...
    if (selectedOperation.toLocaleLowerCase() === 'interest payment') {
      const desc = (accTypeCd == 5 ||accTypeCd == 16)? ' MIS ' : ' Savings ' //marker
...
getAlertIcon(type: MessageType): string {
  ...
  setAccFlag(openDT:any, currDt:any){
    ...
    async getChequeIssueData(acc_cd: number,acc_num:string): Promise<any> {
    const dt = {
      acc_type_cd: acc_cd,
      acc_num: acc_num,
      ardb_cd: this.sys.ardbCD
    ...
    try {
      const res = await this.svc.addUpdDel<any>('Deposit/RetrieveChqDataAll', dt).toPromise();
      console.log(res);
      return res;
    } finally {
      this.isLoading = false;
  ...
  verifyChequeStatus(event:any){
  console.log(event);  


  let status= this.allChequeData.find(x => x.chq_no.replace(/^0+/, '') == event.replace(/^0+/, ''))?.chq_status;
    if(status === "I"){
      this.chequeUseStatus = "Fresh";
    }
    else if(status === "U"){
      this.chequeUseStatus = "Used";
      // this.chequeUseStatus = "Fresh";
      this.td.instrument_num.setValue(0);
    }
    else{
       this.chequeUseStatus = "Wrong";
      //  this.chequeUseStatus = "Fresh";
       this.td.instrument_num.setValue(0);
    }
