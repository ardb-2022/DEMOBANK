 ...
 chequeUseStatus:string=''
  ...
  this.accDtlsFrm.patchValue({
    ...
    total_due: this.fd.curr_intt.value + this.fd.ovd_intt.value + this.fd.penal_intt.value + this.fd.ovd_principal.value + this.fd.curr_principal.value + this.td.paid_amount.value,
          ...
          if(this.isRecovery)
         {
            // if(this.acc2.tmloanall.emi_formula_no==3){
        ...
        //  }
          ...
          // if((this.sys.ardbCD=='26'||this.sys.ardbCD=='1') && acc.tmloanall.acc_cd==20416){
            if(exists){
    ...
    err => { this.isLoading = false; }
      ...
      this.svc.addUpdDel<any>('Loan/GetLoanData', acc1).subscribe(
        async res => {
          ...
          this.acc2 = res;
          if(this.acc2){
             this.allChequeData=await this.getChequeIssueData(acc.tmloanall.acc_cd,acc.tmloanall.loan_id);
...
setACCDtls(acc:any){
  ...
  public onOperationTypeChange(): void {
    ...
    if (selectedOperation.oprn_desc.toLocaleLowerCase() === 'disbursement') {
      ...
      this.setACCDtls(this.acc2);
      this.minDate=new Date(this.sys.CurrentDate)
  ...
  onAmtChng(): void {
    ...
    if((+this.td.amount.value) >(+this.fd.total_due.value)&&(selectedOperation.oprn_desc.toLocaleLowerCase() === 'recovery' ) && (this.f.acc_type_cd.value!=23115)){
      this.HandleMessage(true, MessageType.Error, 'Amount can not be greaterthan due amount.');
      this.td.amount.setValue('');
      return;
    }
    if((+this.td.amount.value) == (+this.fd.total_due.value)&&(selectedOperation.oprn_desc.toLocaleLowerCase() === 'recovery' ) && (this.f.acc_type_cd.value!=23115)){
      this.HandleMessage(true, MessageType.Error, 'Amount can not be equal with due amount.');
      this.td.amount.setValue('');
      return;
    }
    if((+this.td.amount.value) !=(+this.fd.total_due.value)&&(selectedOperation.oprn_desc.toLocaleLowerCase() === 'close' ) ){
      this.HandleMessage(true, MessageType.Error, 'Amount should be equal to due amount.');
      this.td.amount.setValue('');
      return;
    }
    ...
    if (this.td.trans_type_key.value === 'C') {
  ...
  onRecoveryTillDateChng(ev: any): void {
    ...
    // console.log(dt1, dt2)
    // console.log(this.dayDiff(this.td.intt_recov_dt.value, this.td.intt_till_dt.value))
    if (this.td.trans_type_key.value === 'R' ||this.td.trans_type_key.value === 'C') {
  ...
  PopulateRecoveryDetails(callval: any) {
    ...
    this.svc.addUpdDel<any>(this.acc2.tmloanall.emi_formula_no==3?'Loan/CalculateLoanInterestEmi':'Loan/CalculateLoanInterest', this.acc2.tmloanall.emi_formula_no==3?dxEMI:tmDep).subscribe(
  ...
  onSaveClick(): void {
    ...
    const selectedOperation = this.operations.filter(e => e.oprn_cd === +this.f.oprn_cd.value)[0];
    if ( selectedOperation.oprn_desc.toLocaleLowerCase() === 'close' ) {
      const selectedDate: Date = this.td.intt_recov_dt.value;
      ...
      if (selectedDate.getTime() === today.getTime()) {
        ...
        console.log("2");
        this.HandleMessage(true, MessageType.Error, 'Intt Calculation Date Should be Equal with System Date');
        return;
      }
...
debugger;
    if (this.td.recov_type.value == 'A') {
      ...
      var dxEMI={
        ...
        "intt_amt":this.emiRecovIntt,
        "recov_amt":this.emiRecovTot,
        "intt_dt":this.acc2.tmloanall.emi_formula_no==3?Utils.convertStringToDt(this.LoanRepSch[this.LoanRepSch.length-1].due_dt1.toString()):this.td.intt_recov_dt.value
        }
 ...
 addDenomination() {
  ...
  getAccountTypeList() {
    ...
    this.svc.addUpdDel<any>('Mst/GetAccountTypeMaster', null).subscribe(
      res => {
        ...
        this.accountTypeList = this.accountTypeList.filter(c => c.trans_way === 'B' && c.dep_loan_flag==='D' && c.acc_type_cd!==11);
        ...
        if (undefined !== res && null !== res && res.length > 0) {
          ...
          temp_deposit_list.forEach(element => {
            if (element.acc_status === null || element.acc_status.toUpperCase() !== 'C') {
              ...
              tdDefTransTrnsfr.minimum_bal = element.ext_instl_tot;
  ...
  checkDebitBalance(tdDefTransTrnsfr: td_def_trans_trf) {
    ...
    if(tdDefTransTrnsfr.cust_acc_number){
      ...
      debugger
      if((tdDefTransTrnsfr.cust_acc_type=='1'|| tdDefTransTrnsfr.cust_acc_type=='7') && (selectedOperation.oprn_desc.toLocaleLowerCase() === 'recovery'||selectedOperation.oprn_desc.toLocaleLowerCase() === 'close')){
        debugger
        if((+tdDefTransTrnsfr.amount)>tdDefTransTrnsfr.clr_bal ){
          this.HandleMessage(true, MessageType.Error, 'Amount entered is more than the Balance of - '+ tdDefTransTrnsfr.cust_acc_number);
          tdDefTransTrnsfr.amount = 0;
          this.TrfTotAmt=0;
          return;
        }
       else if((+tdDefTransTrnsfr.amount)>(tdDefTransTrnsfr.clr_bal-tdDefTransTrnsfr.minimum_bal) ){
        this.HandleMessage(true, MessageType.Error, 'Freezing A/c Balance is - ₹'+ tdDefTransTrnsfr.minimum_bal)+', Transaction can not proceed';
        tdDefTransTrnsfr.amount = 0;
        this.TrfTotAmt=0;
        return;
      }
