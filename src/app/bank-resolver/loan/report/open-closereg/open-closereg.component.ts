...
export class OpenCloseregComponent implements OnInit,AfterViewInit {
  ...
  displayedColumns_c: string[] = ['sl_no','brn_cd','trans_dt','cust_name','acc_desc', 'loan_id','sanc_dt','sanc_amt','curr_rt','ovd_rt','closing_amt','closing_intt'];
  displayedColumns_o: string[] = ['sl_no','brn_cd','trans_dt','cust_name','acc_desc', 'loan_id','sanc_dt','sanc_amt','disb_amt','instl_no','curr_rt','ovd_rt'];
        ...
        this.svc.addUpdDel('Loan/PopulateLoanCloseRegister', dt).subscribe(data => {
          ...
          this.isLoading=false
            ...
            for(let i=0;i<this.reportData.length;i++){
              ...
              this.reportData[i].brn_name=this.getBranchName(this.reportData[i].brn_cd);
              ...
              this.totSanc_c += this.reportData[i].sanc_amt
              this.totClsPrn += this.reportData[i].closing_amt
              this.totClsIntt += this.reportData[i].closing_intt
            }
        ...
        this.svc.addUpdDel('Loan/PopulateLoanOpenRegister', dt).subscribe(data => {
          ...
          this.isLoading=false
            ...
            for(let i=0;i<this.reportData.length;i++){
              ...
              this.reportData[i].sanc_dt=this.comSer.getFormatedDate(this.reportData[i].sanc_dt);
              this.reportData[i].trans_dt=this.comSer.getFormatedDate(this.reportData[i].trans_dt);
               this.totSanc_o +=this.reportData[i].sanc_amt
                this.totDisb +=this.reportData[i].disb_amt
            }
