...
export class LoanRecoveryConsoComponent {
  ...
  displayedColumns: string[] = ['brn_name','loan_type','activity','loan_id','cust_name','trans_dt','curr_prn_recov','ovd_prn_recov','tot_prn_recov','curr_intt_recov','ovd_intt_recov','tot_intt_recov','total_recov_amt','last_intt_calc_dt'];
  modalRef: BsModalRef;
  isOpenFromDp = false;
  isOpenToDp = false;
    ...
    if (this.reportcriteria.invalid) {
      ...
      this.svc.addUpdDel('Loan/PopulateRecovRegConso',dt).subscribe(data=>{console.log(data)
        ...
        for(let i=0;i<this.reportData.length;i++){
           if(this.reportData[i].last_intt_calc_dt.substr(0,10)=='01/01/0001'){
              this.reportData[i].last_intt_calc_dt='';
            }
            else{
              this.reportData[i].last_intt_calc_dt=this.comSer.getFormatedDate(this.reportData[i].last_intt_calc_dt);
            }
