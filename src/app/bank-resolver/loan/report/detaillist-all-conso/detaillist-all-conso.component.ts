...
@Component({
  selector: 'app-detaillist-all-conso',
  templateUrl: './detaillist-all-conso.component.html',
  styleUrls: ['./detaillist-all-conso.component.css'],
  providers:[ExportAsService]
})
export class DetaillistAllConsoComponent implements OnInit {
...
broFund:any[]=[]
  ...
  displayedColumns: string[] = ['SL_NO','brn_cd','acc_name','party_name', 'acc_num', 'list_dt','curr_intt_rate','plus','curr_prn','curr_intt','computed_till_dt'];
 ...
 onLoadScreen(content) {
  ...
  getOperationMaster(){
    this.isLoading = true;
    if (undefined !== DetaillistAllConsoComponent.operations &&
      null !== DetaillistAllConsoComponent.operations &&
      DetaillistAllConsoComponent.operations.length > 0) {
      this.isLoading = false;
      this.AcctTypes = DetaillistAllConsoComponent.operations.filter(e => e.module_type === 'LOAN')
        .filter((thing, i, arr) => {
      ...
      this.svc.addUpdDel<mm_operation[]>('Mst/GetOperationDtls', null).subscribe(
        res => {
         ...
         if(res){
          DetaillistAllConsoComponent.operations = res;
          this.isLoading = false;
          this.AcctTypes = DetaillistAllConsoComponent.operations.filter(e => e.module_type === 'LOAN')
            .filter((thing, i, arr) => {
              return arr.indexOf(arr.find(t => t.acc_type_cd === thing.acc_type_cd)) === i;
  ...
  public SubmitReport() {
    if (this.reportcriteria.invalid) {
      ...
      this.svc.addUpdDel<any>('Loan/PopulateLoanDetailedListConso',dt).subscribe(data=>{console.log(data)
       const allData=data;
  ...
  applyFilter(event:Event){
    ...
    switch(this.selectedValue1){
      case "Branch": 
      this.filteredArray=this.reportData.filter(e=>e.brn_cd?.toLowerCase().includes(filterValue.toLowerCase())==true)
        break;
        case "Acc Type": 
