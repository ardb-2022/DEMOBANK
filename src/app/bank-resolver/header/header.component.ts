...
maturedData:any[]=[];
  ...
  ngOnInit(): void {
    ...
    localStorage.setItem('W_attempt','0');
...
// this.openModalMessage(this.Notice)
  ...
  checkDate(){
    const diffMs = new Date().getTime() - this.sys.CurrentDate.getTime();
      
      // Convert to days
      const diffDays = diffMs / (1000 * 60 * 60 * 24);
      if((+diffDays)>1){
        // alert("SystemDate and Software Date Mishmach")
        console.log((+diffDays)>1);
        console.log('hello');
        this.HandleMessage(true, MessageType.Warning,'Software date shows a mismatch with today’s date' )
      }
