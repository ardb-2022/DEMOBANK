...
debugger
 ...
 putValue(entry:any,i:any){
  ...
  dateChange(dt1: any) {
  const selectedDate = new Date(dt1);
  const currentDate = new Date(this.sys.CurrentDate);

  // Get the current financial year's start (April 1)
  const financialYearStart = new Date(currentDate.getFullYear(), 3, 1); // month index 3 = April
    debugger
  // If prevStatus = 'Y' and selected date is before 01/04/currentYear
  if (this.sys.prevStatus === 'Y' && selectedDate < financialYearStart) {
    this.HandleMessage(true, MessageType.Error, 'Year End Completed, Voucher can not allow previous year.');
    this._voucherDt = null;
    return;
  }

  // Existing future/same day check
 else if (this.dayDiff(dt1, this.sys.CurrentDate) >= 0) {
